...
void ppmWrite(const char *filename, const int width, const int height, const std::vector<PackedPixel>& pixels) {
  ofstream f(filename, ios::binary);
  f << "P6 " << width << " " << height << " 255\n";
  for (int i = 0; i < height; ++i) {
    f.write(reinterpret_cast<const char*>(&pixels[width*i]), 3*width);
  }
}
